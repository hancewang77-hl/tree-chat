...
| **批注** | 混在对话中 | 命名叶片笔记——两步创建（命名→内容），每节点最多3片，不污染 AI 对话链 |
...
| 🍃 **叶片 (Leaf)** | 两步创建：先命名，再写内容——每节点最多3片，默认不进入AI上下文 |
...
- Context Compiler 按”根任务 → 有效父路径语义 → 显式纳入的 Leaf（名称+内容）→ 当前任务 → 相关资料片段 → 当前问题”统一编译。
...
│   ├── layout/                   # 顶栏、侧栏、底部输入
│   ├── toolbar/                  # 树工具栏、缩放控件
│   └── overlays/                 # 小地图、历史、搜索、对话框、模态传送门
├── hooks/
│   ├── useTreeLayout.ts          # D3 树布局 + 常量（MAX_LEAVES_PER_NODE = 3）
