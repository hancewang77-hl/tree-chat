...
npm test                 # Run Vitest (all projects)
npm run test:unit        # Node-environment logic tests (*.test.ts)
npm run test:component   # jsdom component tests (*.test.tsx)
```

Vitest is split into two projects (`vitest.config.ts`): **unit** (node env; `app/api/**`, `src/**`, `hooks/**` `*.test.ts`) and **component** (jsdom; `*.test.tsx`). Tests are colocated `*.test.ts(x)` files; Vitest aliases `@/` to the project root (same as tsconfig paths).

Component tests render through the real `TreeProvider` via the `src/test/harness/renderWithTree.tsx` helper (seed localStorage → hydrate on mount). Shared setup lives in `src/test/setup/` and mocks in `src/test/mocks/` — note `mocks/storage.ts` installs an in-memory `localStorage`/`sessionStorage` because vitest v4's jsdom ships without Web Storage.
...
└── useResizableSidebar.ts      # Sidebar drag-to-resize (side/min/max opts; wired into Forest + Inspector sidebars)
```
...
- **Load hardening** (persisted state is attacker-influenceable): `normalizeNode` coerces `layer`/`offsetX`/`offsetY` to finite numbers; `loadInitialState` uses `Object.hasOwn` for id lookups (so a `"toString"` id resolves as absent) and a finite `selectedLayer`; `collectSubtreeIds`, `getContextPath`, and `buildHierarchy` carry visited-guards so a cyclic `parentId`/`children` cannot infinite-loop or stack-overflow on load
...
- Before chunk transport Auxo extracts explicit numbered questions into stable source units. Marker detection undoes Turndown's digit-led escapes (`**1\. …**`) except on headings/bullets (chapter numbering stays structure); bare section words (作文/翻译/判断…) demote a line only under Chinese-numeral or heading markers; answer-key sections (参考答案 etc.) produce no units; trailing content-free table rows are trimmed from unit text. The fixed non-thinking `deepseek-v4-flash` planner returns only source-unit IDs and hierarchy, never copied question text or free descriptions.
...
### Tree-metaphor action system (core action groups)
...
| Layer    | `LAYER_MOVE_START` → `LAYER_MOVE_CONFIRM` | Move a node to another layer (`toolMode: "layerMove"`) |
...
`renderMarkdownToHTML` output goes into `dangerouslySetInnerHTML`. Its `escapeHTMLPreserving` helper entity-escapes fenced/inline **code bodies at protection time** (math is left verbatim for KaTeX, which escapes with `trust:false`); Stage 2/5 therefore must not re-escape. This is a security invariant — a malformed fence (single-line, or non-word info string) must never restore raw HTML. `stripMarkdown`'s link/image regexes exclude `]` from the text group so a bracketed label (e.g. `[公式]`) before a link is not swallowed.
...
- **Layer-move mode** (`toolMode: "layerMove"`): Armed by the toolbar 移层 button (auto-switches to 3D) via `LAYER_MOVE_START`. The scroll wheel picks the target layer (`SET_LAYER` mirrors it into `pendingNodeLayer`); the node's ✓ button commits with `LAYER_MOVE_CONFIRM` (one undoable Rings entry). Root/leaf/streaming/same-layer moves are rejected with zero writes, and pruning the moving node cancels the mode.
...
- `POST /api/chat` — per-IP rate limit 30/min. Accepts compiled messages, streams plain-text deltas (`text/plain`, no SSE framing; `max_tokens: 2048`). Non-streaming mode available. Supports `AbortController` cancellation. Body is bounded (content-length reject + streaming bounded read, ≤1MB) and validated (≤200 messages, each `{role, content}` with an allowed role, total content ≤200K chars) before forwarding to the server-held key. 500s return a fixed error string (no upstream detail leaked).
...
- **DOCX** → mammoth.js → HTML → Turndown (+ GFM plugin) → Markdown. Two-layer ZIP-bomb defense: (1) `assertSafeDocxArchive` metadata pre-screen (max 2000 entries, 50MB uncompressed, 20MB per entry, 200× ratio — but these are the archive's *declared* sizes); (2) `assertDocxInflationSafe` streams each entry through `DecompressionStream('deflate-raw')` and aborts the moment the *actual* inflated size passes the per-entry/total cap, catching a bomb that lies about its declared sizes (mammoth's jszip→pako inflates the real stream). `markFirstTableRowsAsHeaders` uses a linear `indexOf` scan (not a lazy regex) to avoid ReDoS on hostile HTML.
- **PDF** → unpdf (PDF.js) extracts text per page. Max 200 pages, 200K chars, 20s extraction timeout, plus a per-page item-count/time guard before materializing a dense page. No OCR — image-only PDFs are rejected.
...
- `useResizableSidebar.ts` is wired into both the Forest sidebar (180–400px) and Inspector sidebar (300–640px); width is session-only (not persisted).
